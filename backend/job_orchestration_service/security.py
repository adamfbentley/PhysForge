...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        # SEC-CFG-002: Explicitly encode SECRET_KEY to bytes
    ...
    # This service relies on 'user_id' for job ownership checks.
    user_id: Optional[int] = payload.get("user_id")
    if user_id is None:
        raise credentials_exception

    # Construct a minimal User object for this service's needs
    # SEC-PTS-001: Pass the original token along with the user object
