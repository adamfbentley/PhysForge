...
# Try to import PySR
try:
    from pysr_discovery import discover_with_pysr, PYSR_AVAILABLE
except ImportError:
...
def process_job(job_id: str, filepath: str):
    ...
    try:
        ...
        # Compute derivatives for both methods
        x_tensor = torch.tensor(x_data, dtype=torch.float32).reshape(-1, 1)
        t_tensor = torch.tensor(t_data, dtype=torch.float32).reshape(-1, 1)
        derivs = compute_derivatives(model, x_tensor, t_tensor)
        
        # Method 1: Sparse regression (fast, linear only)
        equation_str, coefficients, r_squared, all_terms = discover_equation(model, x_data, t_data, job_id=job_id)
        
        # Method 2: PySR symbolic regression (slower, all functions)
        pysr_results = None
        if PYSR_AVAILABLE:
            if job_id:
                processing_status[job_id] = {
                    "stage": "pysr",
                    "progress": "0%",
                    "message": "🔬 Running PySR symbolic regression (2-3 min)..."
                }
            
            try:
                pysr_results = discover_with_pysr(
                    x_data, t_data, u_data,
                    {k: v.detach().numpy().flatten() for k, v in derivs.items()},
                    timeout_seconds=120  # 2 minutes
                )
                
                if job_id and pysr_results.get('success'):
                    processing_status[job_id] = {
                        "stage": "pysr",
                        "progress": "100%",
                        "message": f"✨ PySR found equation: {pysr_results['best_equation']['equation']}"
                    }
            except Exception as e:
                print(f"PySR failed: {e}", flush=True)
                pysr_results = {"success": False, "error": str(e)}
        ...
        result = {
            "sparse_regression": {
                ...
                "mse": float(mse),
                "method": "Sparse Regression (Fast, Linear Terms Only)"
            },
            "pysr_regression": pysr_results if pysr_results and pysr_results.get('success') else None,
            "pinn_training": {
